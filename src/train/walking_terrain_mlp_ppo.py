...
# Allow direct execution like: uv run src/train/walking_terrain_mlp_ppo.py
...
from src.algorithm.ppo import PPOParams, PPOTrainer
...
def _make_wandb_logger(exp_name: str, config: dict) -> WandbLogger:
    # TorchRL versions differ slightly in accepted kwargs, so pass only supported args.
    sig = inspect.signature(WandbLogger)
    kwargs = {
        "exp_name": exp_name,
    ...
    return WandbLogger(**filtered_kwargs)
...
def main() -> None:
    ...
    exp_name = f"walking_terrain_mlp_ppo_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
